...
| Privacidad y ARCO | Parcial | El commit `be15c3b` agrega la ruta `/aviso-de-privacidad` y el enlace desde el pie de página. | Confirmar identidad y domicilio del responsable, correo ARCO, finalidades reales, encargados, transferencias, plazos y operación efectiva de formularios. |
...
| Blog o recursos | Cubierto en frontend | Existe `/blog` con un único artículo completo: “La Soledad del Cuidador”. Las rutas anteriores de Recursos redirigen al blog. | Validación editorial médica/jurídica, autoría definitiva y eventual CMS para publicación continua. |
...
| Guía o recurso descargable | Deshabilitado | No se publican formularios ni archivos descargables. Los PDF anteriores fueron eliminados. | Definir contenido, proveedor y tratamiento de datos antes de implementar nuevamente esta función. |
...
1. Validar con el cliente el contenido del Blog, FAQ, equipo y testimonios.
2. Confirmar responsable de privacidad, operación de contacto y tiempo real de respuesta.
3. Conectar Contacto a un proveedor aprobado; ejecutar la prueba técnica y confirmar recepción real en el buzón o CRM.
